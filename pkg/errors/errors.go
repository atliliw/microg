...
func FromGrpcError(e error) error {
	...
	if strings.HasPrefix(msg, "[CODE:") {
		...
		if end > 6 {
			...
			actualMsg := msg[end+1:]
			if len(actualMsg) > 0 && actualMsg[0] == ' ' {
				actualMsg = actualMsg[1:]
			}
			return &withCode{
				err:  New(actualMsg),
				code: code,
...
func ToGrpcError(e error) error {
	...
	var perr *withCode
	if As(e, &perr) {
		// 把自定义 code 编码到消息中，格式: [CODE:xxx] message
		// 使用 gRPC Unknown 作为标准 code
		msg := fmt.Sprintf("[CODE:%d] %s", perr.code, perr.err.Error())
		err := status.Error(gCode.Unknown, msg)
		return err
	}
