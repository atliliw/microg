...
func (cache CacheStrategy) AuthFunc() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(header) == 0 {
			middlewares.WriteError(c, http.StatusUnauthorized, errors.WithCode(code.ErrMissingHeader, "Authorization header cannot be empty."))
			c.Abort()
		...
		if err != nil || !parsedT.Valid {
			middlewares.WriteError(c, http.StatusUnauthorized, errors.WithCode(code.ErrSignatureInvalid, err.Error()))
			c.Abort()
		...
		if KeyExpired(secret.Expires) {
			...
			middlewares.WriteError(c, http.StatusUnauthorized, errors.WithCode(code.ErrExpired, "expired at: %s", tm))
			c.Abort()
